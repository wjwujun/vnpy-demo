...
class DoubleMa22Strategy(CtaTemplate):
    ...
    close_price=[5,6,7,8,9,10,15,20,25,30,35,40,45,50,60,70,80,90,100,125,200,250,300,350,400,450,500]  #止盈等级，根据等级来确定止盈的价格
    arr_long = []   # 确定止盈的范围
    arr_short = []  # 确定止盈的范围
    stop_long = 0   # 多头止损
    stop_short = 0  # 空头止损
            ...
            if self.direction == Direction.LONG:
                if self.stop_price < 0:  # 盈利
                    ...
                    if self.stop_price >= 10:
                        ...
                        self.stop_long = max(self.current_price - 7, self.stop_long)  # 亏损止损价
            else:
                if self.stop_price < 0:  # 亏损
                    if self.stop_price <= -10:
                        ...
                        self.stop_short = min(self.current_price + 7, self.stop_short)  # 亏损止损价
                else:  # 盈利
                    for i in self.close_price:
